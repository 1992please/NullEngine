...
void FOpenGLFrameBuffer::Invalidate()
{
	...
	ColorAttachments.SetNum(1);
	glCreateTextures(GL_TEXTURE_2D, 1, ColorAttachments.GetData());
	glBindTexture(GL_TEXTURE_2D, ColorAttachments[0]);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, BufferInfo.Width, BufferInfo.Height, 0, GL_RGBA, GL_UNSIGNED_BYTE, nullptr);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	...
	glTexStorage2D(GL_TEXTURE_2D, 1, GL_DEPTH24_STENCIL8, BufferInfo.Width, BufferInfo.Height);
